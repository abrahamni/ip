...
module.exports = function (APP) {
    ...
    APP.FORM.getResponses = function (params, callback) {
        ...
        APP.FORM.getForm(params, function (err, form) {
            ...
            // APP.lib.db.query(APP.SQL.selectFormItems, [params.lang, form.id, null], function (err, items) {
            //     if (err) {
            //         return APP.serverError('APP.FORM.getResponses() > selectFormItems', err, callback); 
            //     }
            ...
            //     APP.AUTH.validUserPermission(params, APP.PERMISSION.COUNCIL, function (err, user) {
            //         if (err || !user) {
            //             items = filterPublicItems(items);
            //         }
            //         var q = buildFormResponsesSelectQuery(params.lang, form, items);
            //         APP.lib.db.query(q.sql, q.values, function (err, responses) {
            //             if (err) {
            //                 return APP.serverError('APP.FORM.getResponses()', err, callback); 
            //             }
            //             callback(null, responses);
            //         });
            //     });
    ...
    APP.FORM.approveResponse = function (params, callback) {
        ...
        updateFormResponseStatus(params, function (err, data) {
            ...
            APP.lib.db.queryOne(APP.SQL.selectCouncilEmail, [params.id], function (err, row) {
                if (row && row.email) {
                    APP.FORM.getFormResponse({ lang: params.lang || 'en', id: params.id }, function (err, formResponse) {
                        var content = '';
                        ...
                        if (formResponse && formResponse.items) {
                            formResponse.items.forEach(function (item) {
                                ...
                                if (item.item_type == 'image') {
                                    content += '<img style="max-width:500px;" src="http://projectname.org/files/' + item.value + '" />';
    ...
    function updateFormResponseActionStatus(params, callback) {
        ...
        APP.AUTH.validUserPermission(params, APP.PERMISSION.COUNCIL, function (err, user) {
            ...
            // TODO: Check if council is updated their report and not other councils.
    ...
    //
    // Attribute-Value tables are hard to deal with SQL language.
    // This helper function creates an SQL query which can be used to select 
    // form responses with item labels as columns and values as rows.
    //
    function buildFormResponsesSelectQuery(lang, form, items) {
        var sql = 'SELECT fr.id, fr.datetime, ';
        var qvIndex = 1;
        var qValues = [lang];

        items.forEach(function (item, i) {
            if (item.type === 'scale' || item.type === 'choice' || item.type === 'location-choice') {
                sql +=  '(' +
                            ' SELECT' +
                            '   fiot.label' +
                            ' FROM ' +
                            ...
                            '           ON fri.option_id = fiot.option_id AND fiot.lang = $1' +
                            ' WHERE ' +
                            '   fri.response_id = fr.id AND fri.item_id = $' + (qvIndex += 1) +
                            ' LIMIT' +
                            '   1' +
                        ') AS "' + item.label + '"';
            } else {
                sql +=  '(' +
                            ' SELECT' +
                            '   (CASE WHEN fri.value::TEXT IS NOT NULL THEN fri.value ELSE frit.value END)' +
                            ' FROM ' +
                            '   form_response_items AS fri' +
            ...
            qValues.push(item.id);
        });

        sql += 'FROM form_responses AS fr WHERE form_id = $' + (qvIndex + 1);
        qValues.push(form.id);

        return { sql: sql, values: qValues };
    }
